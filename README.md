...
- **Timeout**: 3s default (Series 3 응답 지연 대응, with retry mechanism + linear backoff). 사양상 응답 latency는 100ms 이내지만, GS805(Series 3, 단일 스레드) 실측 시 첫 응답이 지연되는 경우가 있어 기본값을 3s로 설정. 재시도 사이 100/200ms backoff.
